...
from common.utils.map_utils import MapSizeParameters, init_map_and_pose_for_env
from common.mapping.categorical_2d_semantic_map_module import MapConstants
...
class Categorical2DSemanticMapState:
    ...
    def __init__(
        ...
        num_channels = self.num_sem_categories + MapConstants.NON_SEM_CHANNELS
    ...
    def get_obstacle_map(self, e) -> np.ndarray:
        ...
        return np.copy(self.local_map[e, MapConstants.OBSTACLE_MAP, :, :].cpu().float().numpy())

    def get_explored_map(self, e) -> np.ndarray:
        """Get local explored map for an environment."""
        return np.copy(self.local_map[e, MapConstants.EXPLORED_MAP, :, :].cpu().float().numpy())

    def get_visited_map(self, e) -> np.ndarray:
        """Get local visited map for an environment."""
        return np.copy(self.local_map[e, MapConstants.VISITED_MAP, :, :].cpu().float().numpy())

    def get_been_close_map(self, e) -> np.ndarray:
        """Get map showing regions the agent has been close to"""
        return np.copy(self.local_map[e, MapConstants.BEEN_CLOSE_MAP, :, :].cpu().float().numpy())

    def get_blacklisted_targets_map(self, e) -> np.ndarray:
        """Get map showing regions the agent has been close to"""
        return np.copy(
            self.local_map[e, MapConstants.BLACKLISTED_TARGETS_MAP, :, :].cpu().float().numpy()
        )
    ...
    def get_semantic_map(self, e) -> np.ndarray:
        ...
        semantic_map[
            MapConstants.NON_SEM_CHANNELS + self.num_sem_categories - 1, :, :
        ] = 1e-5  # Last category is unlabeled
        semantic_map = semantic_map[
            MapConstants.NON_SEM_CHANNELS : MapConstants.NON_SEM_CHANNELS + self.num_sem_categories, :, :
        ].argmax(0)
        return semantic_map
