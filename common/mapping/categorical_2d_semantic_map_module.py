...
from common.mapping.instance_tracking_modules import InstanceMemory
...
class Categorical2DSemanticMapModule(nn.Module):
    ...
    def __init__(
        ...
        dilate_iter: int = 1,
        dilate_size: int = 3,
        target_blacklisting_radius: Optional[int] = None,
        ...
        self.max_mapped_height = int(
            1.2 * self.camera_height / self.z_resolution - self.min_voxel_height
        )
    ...
    def forward(
        ...
        Arguments:
            ...
            init_local_map: initial local map before any updates of shape
             (batch_size, MapConstants.NON_SEM_CHANNELS + num_sem_categories, M, M)
            init_global_map: initial global map before any updates of shape
             (batch_size, MapConstants.NON_SEM_CHANNELS + num_sem_categories, M * ds, M * ds)
        ...
        Returns:
            seq_map_features: sequence of semantic map features of shape
             (batch_size, sequence_length, 2 * MapConstants.NON_SEM_CHANNELS + num_sem_categories, M, M)
        ...
        map_features_channels = 2 * MapConstants.NON_SEM_CHANNELS + self.num_sem_categories
    ...
    def _aggregate_instance_map_channels_per_category(
        ...
        top_down_instance_one_hot = curr_map[
            :,
            (MapConstants.NON_SEM_CHANNELS + self.num_sem_categories) : (
                MapConstants.NON_SEM_CHANNELS + self.num_sem_categories + num_instance_channels
            ),
            :,
            :,
        ]
        ...
        curr_map = torch.cat(
            (
                curr_map[:, : MapConstants.NON_SEM_CHANNELS + self.num_sem_categories],
                top_down_instances_per_category,
                curr_map[
                    :,
                    MapConstants.NON_SEM_CHANNELS
                    + self.num_sem_categories
    ...
    def _update_local_map_and_pose(  # noqa: C901
        ...
        Returns:
            current_map: current local map updated with current observation
             and location of shape (batch_size, MapConstants.NON_SEM_CHANNELS + num_sem_categories, M, M)
        ...
        num_channels = MapConstants.NON_SEM_CHANNELS + self.num_sem_categories
        if self.record_instance_ids:
        ...
        agent_view[:, MapConstants.OBSTACLE_MAP : MapConstants.OBSTACLE_MAP + 1, y1:y2, x1:x2] = fp_map_pred
        agent_view[:, MapConstants.EXPLORED_MAP : MapConstants.EXPLORED_MAP + 1, y1:y2, x1:x2] = fp_exp_pred

        agent_view[:, MapConstants.NON_SEM_CHANNELS :, y1:y2, x1:x2] = (
        ...
        if self.record_instance_ids:
            ...
            current_map[
                :,
                MapConstants.NON_SEM_CHANNELS
                + self.num_sem_categories : MapConstants.NON_SEM_CHANNELS
        ...
        current_map[:, MapConstants.CURRENT_LOCATION, :, :].fill_(0.0)
        ...
        for e in range(batch_size):
            ...
            current_map[
                e,
                MapConstants.CURRENT_LOCATION : MapConstants.CURRENT_LOCATION + 2,
                y - 2 : y + 3,
                x - 2 : x + 3,
            ].fill_(1.0)
            ...
            #     self.draw_line(current_map[e, MapConstants.CURRENT_LOCATION : MapConstants.CURRENT_LOCATION + 2], x, y, self.old_x, self.old_y)
            ...
            try:
                ...
                if blacklist_target:
                    ...
                    current_map[
                        e,
                        MapConstants.BLACKLISTED_TARGETS_MAP,
                        y - radius : y + radius + 1,
                        x - radius : x + radius + 1,
                    ][been_close_disk == 1] = 1
        ...
        if debug_maps:
            ...
            explored = current_map[0, MapConstants.EXPLORED_MAP].numpy()
            been_close = current_map[0, MapConstants.BEEN_CLOSE_MAP].numpy()
            obstacles = current_map[0, MapConstants.OBSTACLE_MAP].numpy()
            plt.subplot(331)
            plt.axis("off")
            plt.title("explored")
            plt.imshow(explored)
            ...
            print("Non semantic channels =", MapConstants.NON_SEM_CHANNELS)
            print("map shape =", current_map.shape)
            breakpoint()

        if self.must_explore_close:
            current_map[:, MapConstants.EXPLORED_MAP] = (
                current_map[:, MapConstants.EXPLORED_MAP] * current_map[:, MapConstants.BEEN_CLOSE_MAP]
            )
            current_map[:, MapConstants.OBSTACLE_MAP] = (
                current_map[:, MapConstants.OBSTACLE_MAP] * current_map[:, MapConstants.BEEN_CLOSE_MAP]
            )
        ...
        for i in range(self.num_sem_categories):
            if (
                ...
                instances = self._update_global_map_instances_for_one_channel(
                    e,
                    global_map[e, MapConstants.NON_SEM_CHANNELS + i + self.num_sem_categories],
                    local_map[e, MapConstants.NON_SEM_CHANNELS + i + self.num_sem_categories],
                    (lmb[e, 0], lmb[e, 1]),
                    (lmb[e, 2], lmb[e, 3]),
                    max_instance_id,
    ...
    def _update_global_map_and_pose_for_env(
        ...
        if self.record_instance_ids:
            ...
            global_map[
                ...
                lmb[e, 0] : lmb[e, 1],
                lmb[e, 2] : lmb[e, 3],
            ] = local_map[e, : MapConstants.NON_SEM_CHANNELS + self.num_sem_categories]
            global_map[
                e,
                MapConstants.NON_SEM_CHANNELS + 2 * self.num_sem_categories :,
                lmb[e, 0] : lmb[e, 1],
                lmb[e, 2] : lmb[e, 3],
            ] = local_map[e, MapConstants.NON_SEM_CHANNELS + 2 * self.num_sem_categories :]
        else:
    ...
    def _get_map_features(self, local_map: Tensor, global_map: Tensor) -> Tensor:
        ...
        Returns:
            map_features: semantic map features of shape
             (batch_size, 2 * MapConstants.NON_SEM_CHANNELS + num_sem_categories, M, M)
        """
        map_features_channels = 2 * MapConstants.NON_SEM_CHANNELS + self.num_sem_categories
        ...
        map_features[:, 0 : MapConstants.NON_SEM_CHANNELS, :, :] = local_map[
            :, 0 : MapConstants.NON_SEM_CHANNELS, :, :
        ]
        # Global obstacles, explored area, and current and past position
        map_features[
            :, MapConstants.NON_SEM_CHANNELS : 2 * MapConstants.NON_SEM_CHANNELS, :, :
        ] = nn.MaxPool2d(self.global_downscaling)(
            global_map[:, 0 : MapConstants.NON_SEM_CHANNELS, :, :]
        )
        # Local semantic categories
        map_features[:, 2 * MapConstants.NON_SEM_CHANNELS :, :, :] = local_map[
            :, MapConstants.NON_SEM_CHANNELS :, :, :
        ]
